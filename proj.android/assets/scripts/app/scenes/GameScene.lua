...
function GameScene:ctor(players)
    ...
    --host countdown
    self.hostCountdown = app:createView("Countdown", 20):align(display.LEFT_CENTER, display.left + imgOffsetX + labelOffsetX, display.cy + imgOffsetY - 20):addTo(self)
    self.hostCountdown:addEventListener("onTimeBurndown", handler(self, self.onTimeBurndown))

    self:setGuest(players[2])
    --guest countdown
    self.guestCountdown = app:createView("Countdown", 20):align(display.RIGHT_CENTER, display.right - imgOffsetX - labelOffsetX, display.cy + imgOffsetY - 20):hide():addTo(self)
    ...
    if app.currentAward ~= nil and app.currentAward.bgHref ~= nil then
        app:loadImageAsync(app.currentAward.bgHref, function(event, texture)
            ...
            awardBg:pos(display.cx, display.cy + imgOffsetY + 30):scale(0.6):addTo(self)
        end)
    end

    local leizhu = display.newSprite("#leizhu.png"):align(display.LEFT_CENTER, display.left + imgOffsetX + 50, display.cy + imgOffsetY + 80):addTo(self)
    ...
    for itemIndex = 1, 4 do
        ...
        end):align(display.CENTER, x, y):addTo(self)
...
function GameScene:setHost(host)
    ...
    self.hostView = app:createView("PlayerView", self.host):imgPos(display.left + imgOffsetX, display.cy + imgOffsetY, true):labelPos(display.LEFT_CENTER, CCPoint(display.left + imgOffsetX + labelOffsetX, display.cy + imgOffsetY + 30)):addTo(self)
end
...
function GameScene:setGuest(guest)
    ...
    self.guestView = app:createView("PlayerView", self.guest):imgPos(display.right - imgOffsetX, display.cy + imgOffsetY, true):labelPos(display.RIGHT_CENTER, CCPoint(display.right - imgOffsetX - labelOffsetX, display.cy + imgOffsetY + 30)):addTo(self)
end
...
function GameScene:showTopic()
    ...
    }):align(display.TOP_LEFT, titleLabelX, titleLabelY):addTo(self)
    ...
    if topic.options then
        for _, item in ipairs(topic.options) do
            ...
            local answerMark = display.newSprite(answerMarkImg, markX, markY):hide():addTo(self)
    ...
    challengeOver:addEventListener("onClose", function(e)
        if self.score > 0 then
            HttpClient.new(function(e)
                self.score = 0 -- reset
            end, getUrl(PRIZE_LIST_URL, app.me.playerId, self.score)):start()
        end
